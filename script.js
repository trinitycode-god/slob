...
(function () {
  ...
  /**
   * Procura, dentro das primeiras linhas do arquivo, a linha que contém o
   * cabeçalho de verdade — assim o arquivo pode ter linhas de instrução,
   * títulos ou linhas em branco antes da tabela (como planilhas com um
   * "passo a passo" no topo) sem quebrar a importação.
   */
  function encontrarLinhaCabecalho(linhasBrutas, colunasEsperadas) {
    const limite = Math.min(linhasBrutas.length, 20);
    for (let i = 0; i < limite; i++) {
      const linha = (linhasBrutas[i] || []).map((v) => String(v).trim());
      const acertos = colunasEsperadas.filter((c) => linha.includes(c)).length;
      if (acertos >= colunasEsperadas.length - 2) return i;
    }
    return -1;
  }

  /** Converte Date, string dd/mm/aaaa ou serial do Excel em objeto Date. */
  function paraData(v) {
  ...
  function faixaPorMeses(meses) {
    if (meses < 18) return "< 18 meses";
    if (meses < 24) return "18-24 meses";
    if (meses < 36) return "24-36 meses";
    if (meses < 48) return "36-48 meses";
    if (meses < 60) return "48-60 meses";
    return "> 60 meses";
  }

  /**
   * Calcula as 5 colunas derivadas (Dias/Meses sem Movimentação,
   * Classificacao SLOB, Faixa Tempo Parado, Valor Total Parado) a partir
   * das 11 colunas brutas do relatório do sistema — não depende mais de
   * fórmula de Excel, então não quebra se uma coluna for apagada por engano.
   */
  function calcularColunasDerivadas(itemBase, hoje) {
    const dataUltMov = paraData(itemBase["Data Ult Mov"]);
    const dias = dataUltMov ? Math.max(0, Math.round((hoje - dataUltMov) / 86400000)) : "";
    const meses = dataUltMov ? Math.floor(dias / 30) : "";
    const classificacao = meses !== "" && meses >= CONFIG.CLASSIFICACAO_LIMITE_MESES ? "SLOB" : "OK";
    const faixa = meses !== "" ? faixaPorMeses(meses) : "";
    const valorTotalParado = paraNumero(itemBase["Custo Total"]);

    return Object.assign({}, itemBase, {
      "Dias sem Movimentação": dias,
      "Meses sem Movimentação": meses,
      "Classificacao SLOB": classificacao,
      "Faixa Tempo Parado": faixa,
      "Valor Total Parado": valorTotalParado
    });
  }
  ...
  function processarArquivo(arquivo) {
    ...
    leitor.onload = (e) => {
      try {
        ...
        // header:1 → array de arrays "cru", sem assumir que a linha 1 é o cabeçalho.
        const linhasBrutas = XLSX.utils.sheet_to_json(primeiraAba, { header: 1, defval: "" });

        const colunasEsperadas = CONFIG.COLUNAS_BASE;
        const indiceCabecalho = encontrarLinhaCabecalho(linhasBrutas, colunasEsperadas);

        if (indiceCabecalho === -1) {
          mostrarToast("Não encontrei o cabeçalho esperado no arquivo. Colunas necessárias: " + colunasEsperadas.join(", "), "erro");
          return;
        }

        const cabecalho = linhasBrutas[indiceCabecalho].map((v) => String(v).trim());
        const posicoes = {};
        colunasEsperadas.forEach((c) => { posicoes[c] = cabecalho.indexOf(c); });
        const faltando = colunasEsperadas.filter((c) => posicoes[c] === -1);
        if (faltando.length) {
          mostrarToast("Colunas não encontradas no arquivo: " + faltando.join(", "), "erro");
          return;
        }

        const linhasDados = linhasBrutas
          .slice(indiceCabecalho + 1)
          .filter((linha) => (linha || []).some((v) => v !== "" && v !== undefined && v !== null));

        if (!linhasDados.length) {
          mostrarToast("O arquivo não tem nenhuma linha de dados abaixo do cabeçalho.", "erro");
          return;
        }
        ...
        state.arquivoAtual = linhasDados.map((linha) => {
          const itemBase = {};
          colunasEsperadas.forEach((c) => {
            let v = linha[posicoes[c]];
            const ehData = c === "Data Ult Mov" || c === "Ult Compra Data" || c === "Ult Saída Data";
            if (ehData) {
              const d = paraData(v);
              itemBase[c] = d ? d.toLocaleDateString("pt-BR") : (v !== undefined ? v : "");
              if (c === "Data Ult Mov") itemBase.__dataUltMovOriginal = v; // preserva pra cálculo
            } else {
              itemBase[c] = v !== undefined ? v : "";
            }
          });
          const item = calcularColunasDerivadas(
            Object.assign({}, itemBase, { "Data Ult Mov": itemBase.__dataUltMovOriginal }),
            hoje
          );
          item["Data Ult Mov"] = itemBase["Data Ult Mov"]; // volta pro formato de exibição dd/mm/aaaa
          delete item.__dataUltMovOriginal;
          return item;
        });

        $("#nomeArquivoSelecionado").textContent = arquivo.name;
        $("#qtdLinhasArquivo").textContent = `${linhasDados.length} linha(s)`;
        $("#areaPreImportacao").style.display = "block";
        $("#areaResultadoImportacao").style.display = "none";
        renderTabela("tabelaPreview", CONFIG.COLUNAS_BANCO_DE_DADOS.map((c) => ({ campo: c, rotulo: c })), state.arquivoAtual.slice(0, 25));
      } catch (err) {
